...
export class PersonalContactController {
  ...
  async createPersonalContact(
    @Res() res,
    @Body() createPersonalContactDTO: CreatePersonalContactDTO,
  ) {
    const personalContact = await this.personalContactService.createPersonalContact(
  ...
  async getProduct(@Res() res, @Param('id') id) {
    const personalContact = await this.personalContactService.getPersonalContact(
      id,
    );
    if (!personalContact)
      throw new NotFoundException('PersonalContact does not exist!');
    return res.status(HttpStatus.OK).json(personalContact);
  }

  @Delete()
  async deletePersonalContact(@Res() res, @Query('id') id) {
    const personalContact = await this.personalContactService.deletePersonalContact(
      id,
    );
    if (!personalContact)
      throw new NotFoundException('PersonalContact does not exist!');
  ...
  async updatePersonalContact(
    ...
    const personalContact = await this.personalContactService.updatePersonalContact(
      id,
      createPersonalContactDTO,
    );
    if (!personalContact)
